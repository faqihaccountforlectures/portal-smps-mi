...
@section('content')
    ...
    {{-- FUNGSI KODE: Menampilkan notifikasi kesalahan jika proses impor berkas CSV guru mengalami kendala --}}
    @if(session('error'))
    ...
    {{-- FUNGSI KODE: Modal Pop-up untuk Mengunggah Berkas CSV / Excel Data Guru --}}
    <div id="teacherImportModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-navy-dark/60 backdrop-blur-sm p-4 transition-opacity duration-300">
        <div class="bg-white rounded-2xl shadow-2xl border border-navy-light/30 max-w-lg w-full overflow-hidden transform transition-all">
            ...
            <div class="px-6 py-5 border-b border-navy-light/20 flex justify-between items-center bg-white-off/40">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-navy-dark font-heading text-base">Impor Data Guru</h3>
                        <p class="text-[11px] text-gray-muted font-medium mt-0.5">Unggah berkas CSV/Excel untuk menambahkan banyak guru sekaligus.</p>
                    </div>
                </div>
                <button type="button" onclick="closeTeacherImportModal()" class="text-gray-muted hover:text-rose-600 p-1.5 rounded-lg hover:bg-rose-50 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <!-- Isi Modal & Formulir -->
            <form action="{{ route('teachers.import') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                @csrf
                ...
                <div class="bg-navy-light/10 border border-navy-light/30 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    ...
                    <a href="{{ route('teachers.template') }}" class="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-lg shadow-sm flex items-center gap-1.5 shrink-0 transition-colors">
                ...
                <div class="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2.5 text-amber-800">
                    ...
                    <p class="text-[11px] leading-relaxed">
                        <b>Tips Microsoft Excel:</b> Agar angka <b>NIP</b> (18 digit) atau <b>No. HP</b> tidak otomatis diubah Excel menjadi format eksponen ilmiah (seperti <i>1.98E+17</i>) dan angka <i>0</i> di awal tidak hilang, awali angka dengan tanda petik satu (contoh: <code class="bg-amber-100 px-1 py-0.5 rounded text-amber-900 font-mono">'19850101...</code> atau <code class="bg-amber-100 px-1 py-0.5 rounded text-amber-900 font-mono">'081234567890</code>). Tanda petik akan dibersihkan otomatis oleh sistem.
                    </p>
                </div>

                <!-- Tombol Aksi Modal -->
                <div class="pt-3 border-t border-navy-light/20 flex justify-end gap-2.5">
                    <button type="button" onclick="closeTeacherImportModal()" class="px-4 py-2 text-xs font-bold text-gray-muted hover:text-navy-dark rounded-xl hover:bg-navy-light/10 transition-colors">
                        Batal
                    </button>
    ...
    {{-- FUNGSI KODE: Skrip kendali interaksi buka/tutup modal impor data guru --}}
    <script>
