...
<div id="deleteModal-{{ $student->id }}" class="hidden fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden text-center relative">
        <div class="p-6">
            ...
            <h3 class="font-bold text-lg text-slate-800 mb-1">Hapus Data Siswa?</h3>
            
            <p class="text-sm text-slate-500 mb-6">Akun milik <b>{{ $student->studentProfile->full_name ?? $student->email }}</b> 
            ...
            <form action="{{ route('students.destroy', $student->id) }}" method="POST" class="flex gap-3 justify-center">
                ...
                <button type="button" onclick="document.getElementById('deleteModal-{{ $student->id }}').classList.add('hidden')" class="px-6 py-2.5 bg-slate-100 text-slate-700 font-semibold text-sm rounded-xl hover:bg-slate-200 transition-colors">
                    Batal
                </button>
